solveLP <- function( cvec, bvec, Amat, maximum=FALSE, maxiter=1000, verbose=FALSE )
{
...
writeMps <- function( file, cvec, bvec, Amat, name="LP" ) {
   ...
   for(i in 1:nVar) {
      ...
      line <- paste( line, paste( rep( " ", 36-nchar(line)-nchar(as.character(cvec[i]))),
         collapse=""), as.character(cvec[i]), sep="")
